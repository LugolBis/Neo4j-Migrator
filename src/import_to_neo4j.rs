                ...
                for column in columns {
                    ...
                    match &column["foreign_key"] {
                        Value::Null => {
                            ...
                            triggers_content.push_str(&format!(r#"CALL apoc.trigger.add('type_{}',"MATCH (m:{}) WHERE m.{} IS NOT NULL AND NOT valueType(m.{}) = '{}' 
                            CALL apoc.util.validate(true, 'ERROR : The type of the field {} need to be a {} .', []) 
                            RETURN m",{{phase: 'before'}});"#,
...
fn extract_nodes(db_neo4j: &Neo4j,tables_path:&str) -> Result<String, String> {
...
pub fn generate_import_files(db_neo4j: &Neo4j, meta_data_path:&str, tables_path:&str, foreign_key_path:&str) -> Result<String, String> {
    ...
    match create_csv_headers(db_neo4j, meta_data_path) {
        Ok(res) => {
            println!("{}",res);
            match extract_nodes(db_neo4j, tables_path) {
                Ok(res) => {
                    println!("{}",res);

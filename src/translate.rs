...
fn from_table_factor(table_factor: TableFactor, hashmap: &mut HashMap<String,String>) -> Result<(),()> {
    ...
    if let TableFactor::Table {name,alias,args,with_hints,version,with_ordinality,partitions,json_path,sample,index_hints}
    = table_factor
    {
        ...
        if let Some(identifier) = name.0[0].clone().as_ident() {
            hashmap.insert(res_alias.clone(), format!("({}:{})",res_alias,identifier.value.to_uppercase()));
            Ok(())
        }
        else { Err(()) }
    }
    ...
    if let JoinConstraint::On(Expr::BinaryOp { left, op, right }) = join_constraint {
        if let Expr::CompoundIdentifier(vector1) = *left {
            if let Expr::CompoundIdentifier(vector2) = * right {
                ...
                Ok((vector1[0].value.clone(),vector1[1].value.clone(),vector2[0].value.clone(),vector2[1].value.clone()))
            }
            else { Err(()) }
        }
        else { Err(()) }
                ...
                JoinOperator::Inner(join_constraint) => {
                    if let Ok(result) = from_join_constraint(join_constraint) {
                        ...
                        if let Some(match_clause) = hashmap.get_mut("match") {
                            match_clause.push_str(&format!("{}-[r{}{}{}:{}__REF__{}]-{},",node1,result.0,result.2,result.3,label1,result.1,node2));
                        }
                        Ok(())
                    }
                    else { Err(()) }
            ...
            match *body {
                SetExpr::Select(select) => {
                    ...
                    if let Err(_) = from_statement(select.from, &mut hashmap) {
                        return Err(String::from("ERROR : when try to transform the 'FROM' clause."))
                    }
                    if let Err(_) = select_statement(select.projection, &mut hashmap) {
                        return Err(String::from("ERROR : when try to transform the 'SELECT' clause."))
                    }
                    ...
                    for (key, value) in hashmap.iter() {
                        if !KEYWORDS.contains(&String::clone(key).as_str()) && !result.contains(value) {
                            println!("\nValue : {value}");
                ...
                _ => return Err(String::from("Your query is not yet supported by the funtion."))
            }
        }
